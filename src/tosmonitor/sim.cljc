...
(defn- run-check!
  ...
  (let [request (merge {:op :tos/change-proposal :subject "siemens-healthineers-tos-1"
                        :candidate candidate}
                       extra)
...
(defn -main [& _]
  (let [db    (store/seed-db)
    ...
    (let [db2 (store/seed-db)
          a2  (op/build db2)
          r   (g/run* a2 {:request {:op :tos/change-proposal :subject "siemens-healthineers-tos-1"
